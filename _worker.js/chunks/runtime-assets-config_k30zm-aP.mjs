...
import { c as createComponent, aB as isHTMLString, r as renderComponent, a as renderTemplate, j as renderUniqueStylesheet, k as renderScriptElement, u as unescapeHTML, l as createHeadAndContent, b as createAstro } from './astro/server_BwhXqllw.mjs';
import { $ as $$Image } from './_astro_assets_BR-HtfSF.mjs';
  ...
  for (let pos = start; pos < content.length; pos++) {
    ...
    switch (state) {
      ...
      case 2 /* escape */:
        ...
        break;
      case 0 /* normal */:
        if (char === '"') state = 1 /* string */;
        else if (content.startsWith(CLOSE, pos)) return pos;
    }
  }
  return null;
}
function containsMarkdocTagInUrl(content, allowedProtocols) {
...
function formatScalar(v) {
  ...
  if (typeof v === "object") {
    return "{" + Object.entries(v).map(
      ([key, value]) => `${isIdentifier(key) ? key : `"${key}"`}: ${formatScalar(value)}`
    ).join(SEP) + "}";
  }
...
function* formatNode(n, o = {}) {
  ...
  switch (n.type) {
    ...
    case "blockquote": {
      ...
      const inner = n.children.map((child) => format(child, { ...no, indent: 0 }).trim()).join(NL + NL);
    ...
    case "list": {
      const isLoose = n.children.some(
        (n2) => n2.children.some((c) => c.type === "paragraph")
      );
...
function annotate(node2, attributes) {
  for (const attribute of attributes) {
    ...
    if (type === "attribute") {
      ...
      if (node2.attributes.class) node2.attributes.class[name] = value;
...
function dynamic(node2, React, { components = {}, resolveTagName = tagName } = {}) {
  ...
  function render3(node3) {
    ...
    return React.createElement(
      ...
      Object.keys(attrs).length == 0 ? null : deepRender2(attrs),
      ...children.map(render3)
    );
  }
...
function block(state, startLine, endLine, silent) {
  ...
  const text2 = state.getLines(startLine, startLine + lines, 0, false);
  const content = text2.slice(text2.indexOf(OPEN) + OPEN.length, findTagEnd(text2)).trim();
...
// src/tokenizer/plugins/link.ts
var INVALID_HREF_MESSAGE = "The 'href' format cannot contain Markdoc tag or variable. URLs must be static strings.";
function pushHrefError(token) {
  const tokenWithErrors = token;
...
function plugin4(md, options) {
  md.core.ruler.push("link_url_validation", (state) => core2(state, options));
}
...
function toValidateError(node2, error2) {
  if (hasValidLocation(error2)) {
    return {
      type: node2.type,
      lines: [error2.location.start.line, error2.location.end.line],
      location: { file: node2.location?.file, ...error2.location },
      error: error2
    };
  }
  return { type: node2.type, lines: node2.lines, location: node2.location, error: error2 };
}
function validateTree(content, config) {
  const output = [...walkWithParents(content)].map(([node2, parents]) => {
    ...
    if (isPromise(errors)) {
      return errors.then((e) => e.map((error2) => toValidateError(node2, error2)));
    }
...
}, "/home/runner/work/malipetek.github.io/malipetek.github.io/node_modules/.pnpm/@astrojs+markdoc@0.15.11_@types+react@19.3.0_astro@5.18.2_@types+node@24.13.4_idb-keyval@6.3._vn2opvb3il6j3lg6of5cxbqpta/node_modules/@astrojs/markdoc/components/Renderer.astro", void 0);
