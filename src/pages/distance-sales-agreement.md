---
layout: ../layouts/BaseLayout.astro
title: Distance Sales Agreement
---

This distance sales agreement applies to fixed-scope remote software implementation packages sold through Malipetek.dev.

## Seller
Seller: Muhammet Ali Petek / Malipetek.dev

Website: https://malipetek.dev

Email: malipetek@gmail.com

## Buyer
The buyer is the person or organization placing an order through the website, email, or written order flow.

## Product or Service
The purchased item is the fixed-scope digital service package shown on the relevant store page. Each package page includes the package name, listed price, delivery estimate, included deliverables, required buyer inputs, and excluded work.

## Price and Payment
The listed price appears on the package page and checkout page in TRY. Payment details are confirmed in writing before work starts. If the package is replaced by a custom scope, the written custom scope controls the commercial terms.

## Delivery
Delivery is remote and digital. Delivery may be made by email, repository pull request, deployed URL, written handoff note, or other written digital handoff agreed for the package.

## Right of Withdrawal and Cancellation
Cancellation and refund rules are described in the Delivery, Refund, and Cancellation Policy. If work has not started, the buyer can request cancellation by email. After work starts, refund eligibility depends on the delivered and undelivered scope.

## Disputes
The governing law is the law of Turkey unless a mandatory rule requires otherwise.

## Contact
Email: malipetek@gmail.com

Effective date: 2026-05-23
